...
export const useRoomsMainStore = create<RoomsMainState>((set, get) => ({
        ...
        set({
            rooms: get().rooms.map((room) => {
                ...
                const unique = Array.from(
                    new Map(merged.map((chat) => [chat._id, chat])).values(),
                );
                return {
                    ...room,
                    page: result.page,
    ...
    handleRealtimePayload: (roomId, payload, currentUserName) => {
        ...
        const nextKeys = eventKey
            ? [...previousKeys, eventKey].slice(-300)
            : previousKeys;
                ...
                .rooms.map((room) => {
                    ...
                    if (payload.action === "add") {
                        ...
                        const nextUnread =
                            ...
                            payload.chat.pengirim.nama !== currentUserName
                                ? room.unread + 1
                                : room.unread;
                        const nextChats = normalizeChats([
                            ...currentChats,
                            payload.chat as RoomChatItem,
                        ]);
                        ...
                        return {
                            ...
                            lastchat: {
                                ...
                                totalReadersTarget:
                                    payload.chat.totalReadersTarget,
                    ...
                    if (payload.action === "seen") {
                        const seenTimestamp = new Date(
                            payload.seenUser.timestamp,
                        ).toISOString();
                        const isCurrentUserSeen =
                            payload.seenUser.user.nama === currentUserName;
                        return {
                            ...room,
                            chats: (room.chats ?? []).map((chat) => {
                                if (!payload.chatIds.includes(chat._id))
                                    return chat;
                                const alreadySeen = (chat.seenUsers ?? []).some(
                                    (item) =>
                                        item.namaUser ===
                                        payload.seenUser.user.nama,
                                );
                            ...
                            lastchat:
                                room.lastchat &&
                                payload.chatIds.includes(room.lastchat._id)
                                    ? {
                                          ...room.lastchat,
                                          seenUsers:
                                              room.lastchat.seenUsers + 1,
                                      }
                                    : room.lastchat,
